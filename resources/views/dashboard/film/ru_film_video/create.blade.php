    ...
    <main id="main" class="main">
        ...
        <section class="section dashboard">
            <div class="card col-8 offset-2">
                <div class="card-body">
                    <h5 class="card-title">Films Form only russion film create</h5>

                    <!-- Vertical Form -->
                    <form id="form" class="row g-3" action="{{ route('RussianFilmVideo.store', $ru_film->id) }}"
                        enctype="multipart/form-data" autocomplete="off" method="POST">
                        @csrf
                        <div class="col-12">
                            <label for="inputSerialName4" class="form-label">Film janri : {{ $ru_film->category->name_uz }}
                                | {{ $ru_film->category->name_ru }}</label>
                            <input type="numeric" name="category" hidden value="{{ $ru_film->category->id }}"
                        ...
                        <div class="col-12">
                            <div class="col-12">
                                <label for="inputSerialName4" class="form-label">Film brendi :
                                    {{ $ru_film->brand->name }}</label>
                                <input type="numeric" name="brand" hidden value="{{ $ru_film->brand->id }}"
                                    class="form-control" id="inputNanme4">
                                @error('brand')
                                    <div class="alert alert-danger mt-1">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-12">
                            <input type="hidden" id="film_id" value="{{ $ru_film->id }}">
        ...
        <script>
            ...
            film_part.addEventListener('input', function(event) {
                ...
                // So'rovni yuborish
                axios.get('http://127.0.0.1:8000/api/russion_video_suitable_recipient', {
                        params: {
                            film_id: filmId,
                            part: value
                        }
