        ...
        <div class="pagetitle">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('RussianFilmVideo.create', $id) }}"> {{ $film->name_uz }} |
                            {{ $film->name_ru }} filmni yaratish</a></li>
                </ol>
            </nav>
        </div><!-- End Page Title -->
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <table class="table">
                                ...
                                <tbody id="">
                                    @if ($russian_film_videos && count($russian_film_videos) > 0)
                                        @foreach ($russian_film_videos as $item)
                                            <tr>
                                                <td>{{ $item->part }}</td>
                                                <td>
                                                    <a href="{{ $item->ru_film_path() }}{{ $item->path }}">
                                                        <video src="{{ $item->ru_film_path() }}{{ $item->path }}"
                                                            class="img img-thumbnail" width="30" style="height: 30px">
                                                    </a>
                                                </td>
                                                <td>
                                                    <a
                                                        href="{{ route('RussianFilmVideo.edit', $item->id) }}"class="badge bg-info">
                            ...
                            @include('dashboard/film/ru_film_video/action/action')
                        </div>
                    </div>
